...
//		System.out.println("attach:"+p.toString());
    ...
    public final Production parse(String sent[]) {
        ...
        for (int i = 0; i < nonTerminals.size(); i++)
            System.out.println("Non Terminal::" + nonTerminals.get(i));
        ...
        all_parse_trees.add(single_parse_tree);
//        Vector<Production> new_parse_tree = new Vector<>();
//        new_parse_tree.addAll(single_parse_tree);
        parseTree(0, null, 0, "S");


        System.out.println("Parse Tree found::");
        for (int i = 0; i < single_parse_tree.size(); i++) {
            System.out.println(single_parse_tree.get(i).toString());
        }

        System.out.println("All Parse Trees::");
        for (int i = 0; i < all_parse_trees.size(); i++) {
            System.out.println("Parse Tree Number::" + i);
            for (int j = 0; j < all_parse_trees.get(i).size(); j++) {
                System.out.println(all_parse_trees.get(i).get(j).toString());
            }
        }
    ...
    private void parseTree(int tree_number, Production parent_production, int start, String symbol_left) {
        Vector<Production> matching_production_rules = getMatchingProductions(final_productions, start, symbol_left);
        Production production = null;
        for (int k = 0; k < matching_production_rules.size(); k++) {
            if (parent_production == null || !matching_production_rules.get(k).equals(parent_production)) {
                if (production != null) {
                    Vector<Production> new_parse_tree = new Vector<>();
                    for (int z = 0; z < all_parse_trees.get(tree_number).size(); z++) {
                        new_parse_tree.add(new Production((Production) all_parse_trees.get(tree_number).get(z)));
                    }
                    all_parse_trees.add(new_parse_tree);
                    tree_number++;
                    production = matching_production_rules.get(k);
                    if (parent_production == null || production != null) {
                        production.parent = parent_production;
                        new_parse_tree.add(production);
                        ...
                        for (int i = 0; i < production.right.length; i++) {
                            if (!isNonTerminal(production.right[i])) {
                                ...
                                System.out.println("\n\n\n\n");
                                System.out.println("Calling parse with tree number:: " + tree_number + " Production:: " + production.toString() + " start value:: " + (production.start + i) + " left symbol:: " + production.right[i]);
                                parseTree(tree_number, production, (production.start + i), production.right[i]);
                            }
                        }
                    }
                } else {
                    ...
                    if (parent_production == null || production != null) {
                        production.parent = parent_production;
                        all_parse_trees.get(tree_number).add(production);
                        if (parent_production != null) {
                            if (parent_production.children == null) {
                                parent_production.children = new ArrayList<>();
                            }
    ...
    private Vector<Production> getMatchingProductions(Vector production_vector, int start, String left) {
        Vector<Production> productions = new Vector<>();
        for (int j = 0; j < production_vector.size(); j++) {
            Production production = (Production) production_vector.get(j);
            if (production.start == start && production.left.equalsIgnoreCase(left)) {
               /* production_vector.remove(production);
                j--;*/
